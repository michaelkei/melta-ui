...
- **`foundations/prohibited.md`を読まずにレビューする**: CLAUDE.mdだけでは禁止パターン76項目の全容が把握できない。Step 2のDSリファレンス読み込みを省略するとAI生成パターンの見逃しが多発する
- **推奨事項を違反として報告してしまう**: `severity-rules.md`の「推奨」と「必須」の区別を確認せずに報告すると偽陽性が大量に出る。特に`text-xs`やlabel要素の文脈判定を見落としやすい
- **サマリーと本文の件数不一致**: レポート生成時にCritical/High/Medium/Lowの件数を手動で数えるとズレが起きやすい。Step 5のサマリー整合チェックを省略しないこと
- **HTMLファイル以外を渡される**: JSXやVueテンプレートを渡されることがある。対象はHTMLファイルのみ。HTMLでない場合は「HTMLファイルを指定してください」と返すこと
