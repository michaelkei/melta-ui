...
- **3箇所同時更新を忘れる**: `foundations/prohibited.md`・`CLAUDE.md`・関連`components/*.md`の3箇所を全て更新する必要がある。1箇所だけ更新すると不整合が発生し、次回のdesign-reviewで検出漏れや誤検知が起きる
- **`foundations/prohibited.md`が存在しない**: melta-uiリポジトリが初期状態だとファイル自体がない場合がある。Step 3-1のテンプレートで自動作成するが、ユーザーに報告してから作成すること
- **具体的なTailwindクラスなしで登録してしまう**: 「カラフルすぎる」等の曖昧な表現だけで禁止ルールを登録すると、design-reviewで判定できない。必ず具体的なクラス名（`border-l-4`、`bg-gradient-to-r`等）を特定してから登録する
- **既存ルールとの部分重複を見落とす**: Step 2の重複チェックを省略すると、同じパターンが異なる表現で2行登録される。`prohibited.md`をGrepで確認してから追加すること
